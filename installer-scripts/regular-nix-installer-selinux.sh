...
bash <(curl -s https://raw.githubusercontent.com/MC-Escherichia/nix-installer-scripts/main/other-scripts/nix-sudo-path.sh)
...
curl /etc/nix https://raw.githubusercontent.com/MC-Escherichia/nix-installer-scripts/main/other-files/nix.conf > /tmp/nix.conf
sudo mv /tmp/nix.conf /etc/nix/nix.conf
sudo rm -f /etc/profile.d/nix-app-icons.sh
sudo curl https://raw.githubusercontent.com/MC-Escherichia/nix-installer-scripts/main/other-files/nix-app-icons.sh > /tmp/nix-app-icons.sh
sudo mv /tmp/nix-app-icons.sh /etc/profile.d/nix-app-icons.sh
...
#curl -s https://raw.githubusercontent.com/MC-Escherichia/nix-installer-scripts/main/nix-out-of-default/setup.sh | bash -s /usr
...
#bash <(curl -s https://raw.githubusercontent.com/MC-Escherichia/nix-installer-scripts/main/backup-scripts/create-backup-selinux.sh)
